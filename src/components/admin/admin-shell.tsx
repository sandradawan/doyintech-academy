...
import { seedDemoStudentsIfEmpty } from "@/lib/admin-crm";
    ...
    return (
      <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center px-4 py-16">
        ...
        <h1 className="mt-2 font-display text-2xl font-semibold tracking-tight">
          Doyintech Academy console
        </h1>
        <p className="mt-2 text-sm text-muted">
          Manage students, payments, content, enrollments, and waitlist.
        </p>
        <form onSubmit={onLogin} className="mt-8 space-y-4 rounded-xl border border-border bg-surface p-6">
          ...
          <button
            type="submit"
            className="flex h-11 w-full items-center justify-center rounded-md bg-primary text-sm font-semibold text-primary-fg"
          >
            Sign in to CRM
          </button>
          <p className="text-xs text-subtle">
            Default PIN is in <code className="rounded bg-surface-2 px-1">src/lib/admin.ts</code> —
            change before production.
          </p>
        </form>
        <Link href="/" className="mt-6 text-center text-sm text-primary hover:underline">
          ← Back to site
        </Link>
      </main>
    );
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-[#0B0E14] text-white">
      <div className="flex h-16 items-center border-b border-white/10 px-4">
        <Logo invert />
        <span className="ml-2 rounded bg-orange/20 px-1.5 py-0.5 text-[10px] font-bold tracking-wide text-orange uppercase">
          CRM
        </span>
      </div>
      ...
      <div className="border-t border-white/10 p-3">
        <Link
          href="/"
          className="mb-1 flex items-center gap-2 rounded-md px-3 py-2 text-sm text-white/65 hover:bg-white/5 hover:text-white"
        >
          View site
        </Link>
        <button
      ...
      <div className="md:pl-60">
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-surface/95 px-4 backdrop-blur">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-muted">Doyintech Academy operations</p>
          </div>
        </header>
