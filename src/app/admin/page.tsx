...
export default function AdminOverviewPage() {
  const [stats, setStats] = useState({
    students: 0, active: 0, payments: 0, revenue: 0, waitlist: 0, overrides: 0,
    lessons: 0, courses: 0, enrollments: 0, uniqueEnrolled: 0, quizAttempts: 0, quizPassRate: 0,
  });
  ...
  async function load(silent = false) {
    ...
    try {
      const [students, payments, waitlist, overrides, acts, enrollments, quizzes] = await Promise.all([
        fetchCrmStudents(), fetchCrmPayments(), fetchWaitlistDb(), fetchContentOverrides(),
        fetchCrmActivity(12), fetchEnrollmentMap(), fetchQuizAttempts(200),
      ]);
      const pay = paymentStatsFrom(payments);
      const lessonTotal = courses.reduce((n, c) => n + courseLessonCount(c), 0);
      const bySlug = new Map<string, number>();
      for (const e of enrollments) bySlug.set(e.courseSlug, (bySlug.get(e.courseSlug) || 0) + 1);
      const top = courses
        .map((c) => ({ slug: c.slug, title: c.title, count: bySlug.get(c.slug) || 0 }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 5);
      const passRate = quizzes.length === 0 ? 0 : Math.round((quizzes.filter((q) => q.passed).length / quizzes.length) * 100);
      setStats({
  ...
  useRealtimeSync(
    ["profiles", "enrollments", "payments", "quiz_attempts", "waitlist", "admin_activity", "content_overrides"],
    () => { void load(true); },
  );
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Live CRM metrics from Supabase — auto-refreshes on changes.</p>
        </div>
      ...
      {error ? (
        <p className="rounded-lg border border-orange/40 bg-orange/10 px-4 py-3 text-sm text-orange">
          {error} — confirm CRM migrations, realtime publication, and admin role.
        </p>
      ) : null}

      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {kpis.map((k) => (
          <Link key={k.label} href={k.href} className="rounded-xl border border-border bg-surface p-5 transition-colors hover:border-primary/40">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <section className="rounded-xl border border-border bg-surface p-5">
          <div className="flex items-center justify-between">
            ...
            <Link href={adminHref("/enrollments")} className="text-xs font-medium text-primary hover:underline">Enrollments</Link>
          </div>
                ...
                return (
                  <li key={c.slug}>
                    <div className="flex items-center justify-between gap-2 text-sm">
                      ...
                      <span className="shrink-0 tabular-nums font-semibold text-primary">{c.count}</span>
                    </div>
        ...
        <section className="rounded-xl border border-border bg-surface p-5">
          <div className="flex items-center justify-between">
            ...
            <Link href={adminHref("/payments")} className="text-xs font-medium text-primary hover:underline">View all</Link>
          </div>
          {recentPay.length === 0 ? (
            <p className="mt-6 text-sm text-muted">No payments yet.</p>
          ) : (
