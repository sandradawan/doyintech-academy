...
export default function AdminStudentsPage() {
  ...
  function changeStatus(id: string, status: CrmStudent["status"]) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          CRM student directory. Wire to Supabase profiles for production multi-admin.
        </p>
      </div>
            ...
            {filtered.map((s) => (
              <tr key={s.id} className="hover:bg-surface-2/40">
                <td className="px-4 py-3">
                  ...
                  <p className="mt-0.5 font-mono text-[10px] text-subtle">{s.id}</p>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <select
                    value={s.status}
                    onChange={(e) => changeStatus(s.id, e.target.value as CrmStudent["status"])}
